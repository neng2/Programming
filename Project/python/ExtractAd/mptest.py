...
def result_check_process(result_queue,CMinfo,firstframelst,ignore_duration,fnt,frames,frames_for_SBD,ignore_scene_change_until,flag,prev_i):
    print("Result process start")
    while True:
        if not result_queue.empty():
            ...
            if result is not None:
                if len(CMinfo) != 0 and CMinfo[-1] == min_indice and firstframeIndex - firstframelst[-1] < ignore_duration * 30:
                    ...
                    ignore_scene_change_until.value = firstframeIndex + ignore_duration * 30
                    CMinfo.append(min_indice)
                    firstframelst.append(firstframeIndex)    
                    print(f"프레임 {firstframeIndex}에서 광고 {min_indice} 검출   추출 시점: frmaeNo {fnt.value} 입력 시점: frmaeNo {fnt_before}") 
                    frames = mp.Manager().list()
                    frames_for_SBD = mp.Manager().list()
                    # frames[cutindex:] = [[dummy_frame, -1] for _ in range(len(frames) - cutindex)]  
                    prev_i.value = firstframeIndex
                    flag.value = False
...
def main():
    ...
    for input_file in input_files:
        ...
        firstframeIndex = 0
        ignore_duration = 14
        fnt_for_check = mp.Value("i",0)
        flag = mp.Value(ctypes.c_bool,False)
        prev_i = mp.Value("i",0)
        ignore_scene_change_until = mp.Value("i",-10)
        manager = mp.Manager()
        frames = manager.list()
        frames_for_SBD = manager.list()
        ...
        process.start()
        result_process = mp.Process(target = result_check_process, args=(result_queue,CMinfo,firstframelst,ignore_duration,fnt_for_check,frames,frames_for_SBD,ignore_scene_change_until,flag,prev_i))
        ...
        with torch.inference_mode():
            ...
            while cap.isOpened() :
                ret, frame = cap.read()
                if not ret and flag.value: # 동영상 프레임 더 이상 안들어오
                ...
                if fnt > ignore_scene_change_until.value-frame_chunksz: #마지막 광고 검출 후 마지막 광고 첫 프레임 이후로 광고 길이만큼 안지났으면 검출 미시행
                    flag.value = True
                
                if flag.value:
                    fnt += 1
                    fnt_for_check.value += 1
